...
public class ConfigManagerFileImpl implements ConfigManager, BasicReloadable {
    ...
    public ConfigManagerFileImpl(GrimAPI api) {
        this.api = api;
        config = new DynamicConfig();
    }

    private File getConfigFile(String path) {
        return new File(api.getGrimPlugin().getDataFolder(), path);
    }

    @Override
    public void reload() {
        api.getGrimPlugin().getDataFolder().mkdirs();
        if (!initialized) {
            initialized = true;
    ...
    private void upgrade() {
        File config = new File(api.getGrimPlugin().getDataFolder(), "config.yml");
        if (config.exists()) {
            try {
    ...
    private void removeLegacyTwoPointOne(File config) throws IOException {
        ...
        Files.move(config.toPath(), new File(api.getGrimPlugin().getDataFolder(), "config-2.1.old.yml").toPath());
    }
    ...
    private void addMissingPunishments() {
        File config = new File(api.getGrimPlugin().getDataFolder(), "punishments.yml");
        String configString;
        if (config.exists()) {
            try {
    ...
    private void fixBadPacketsAndAdjustPingConfig(File config, String configString) {
        ...
        File punishConfig = new File(api.getGrimPlugin().getDataFolder(), "punishments.yml");
    ...
    private void newOffsetNewDiscordConf(File config, String configString) throws IOException {
        ...
        File discordFile = new File(api.getGrimPlugin().getDataFolder(), "discord.yml");
    ...
    private void addSuperDebug(File config, String configString) throws IOException {
        ...
        File messageFile = new File(api.getGrimPlugin().getDataFolder(), "messages.yml");
