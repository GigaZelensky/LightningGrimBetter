...
public class SprintB extends Check implements PostPredictionCheck {
    ...
    public void onPredictionComplete(final PredictionComplete predictionComplete) {
        if (player.isSlowMovement && player.sneakingSpeedMultiplier < 0.8f && predictionComplete.isChecked()) {
            ...
            if (version.isNewerThanOrEquals(PacketClientVersions.V_1_14) && player.wasFlying && player.lastPose == Pose.FALL_FLYING && !player.isGliding) {
                return;
            }

            // https://github.com/GrimAnticheat/Grim/issues/1948
            if (version == PacketClientVersions.V_1_21_4 && (Collections.max(player.uncertaintyHandler.pistonX) != 0
