...
/**
 * FastClick – production build (based on FastPlace)
 *
 * Behaviour
 * ─────────
 ...
 * • Debug stream gated by "grim.debug.fastclick".
 */
@CheckData(name = "FastClick", experimental = true)
public class FastClick extends Check implements PacketCheck {
    ...
    public FastClick(@NotNull GrimPlayer player) { super(player); }
    ...
    /* ---------------- core ---------------- */
    private void handle(long now, PacketReceiveEvent event) {

        final boolean debug = player.hasPermission("grim.debug.fastclick");
        ...
        if (lastPacketTime != -1L) {
            ...
            if (deltaNs > gapNs) {
                if (debug) player.sendMessage(String.format("[A] gap %.2f ms (reset)",
                        deltaNs / 1_000_000D));
                ...
                if (debug) player.sendMessage("[A] Δ=" + deltaNs / 1_000_000D + " ms");
            }
        }
